...
class TableGenHtml(TableGen):
    ...
    def __init__(self, title='', completedoc=False):
        ...
        if self.completedoc:
            ...
            self.table = self.html.body.table(border=BORDER_SIZE)
        else:
            self.html = html.HTML()
            self.table = self.html.table(border=BORDER_SIZE)
...
Please install either to use csv2html.")
    ...
    try:
        print("in file:", args.inputfile)
        print("out file:", args.outputfile)
